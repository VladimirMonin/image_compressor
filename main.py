...
OUTPUT_EXTENSIONS = {"WEBP": ".webp", "HEIF": ".heic", "AVIF": ".avif", "JPEG": ".jpg"}
...
def prepare_compression_tasks(
    image_files: List[str], format_type: str, quality: int
...
def process_with_multiprocessing(
    input_path: str, format_type: str = OUTPUT_FORMAT, quality: int = QUALITY
) -> None:
    """
...
def get_user_format_choice() -> str:
    ...
    choice = input("\nВведите номер формата (1-4) или Enter для WEBP: ").strip()
...
def get_user_quality() -> int:
    ...
    while True:
        quality_input = input(
            f"\nВведите качество (0-100) или Enter для {QUALITY}: "
        ).strip()
